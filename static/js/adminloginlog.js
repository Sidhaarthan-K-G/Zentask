$(document).ready(function () {
    ...
    //     const loginID = row.attr("data-login_id");
    
    //     if (!loginID) {
    //         showFlashMessage("Missing login ID.", "error");
    //         return;
    //     }
    ...
    //             data: JSON.stringify({ login_id: loginID }), // ✅ fixed here
    //             success: function () {
    //                 row.remove();
    ...
    // Fetch login data and populate table
    async function fetchloginLog() {
        const tableSelector = "#logintable";
        let spinnerTimeout = setTimeout(() => {
            showSpinnerInTable(tableSelector, 2);
        }, 300);

        try {
            const response = await $.ajax({
                url: "/api/get_login_log", // Make sure this API exists
                method: "GET",
                dataType: "json"
            });

            clearTimeout(spinnerTimeout);
            showSpinnerInTable(tableSelector, 2); // brief spinner
            await new Promise(resolve => setTimeout(resolve, 1500)); // smooth loading
            clearTableBody(tableSelector);

            const loginData = response?.login_table || [];

            if (loginData.length > 0) {
                loginData.forEach(row => {
                    const tr = `
                        <tr data-login_id="${row.login_id}">
                            <td>${row.email}</td>
                            <td>${row.password}</td>
                        </tr>
                    `;
                ...
                $(`${tableSelector} tbody`).html(`
                    <tr><td colspan="2" style="text-align:center; color:#ccc;">No login records found.</td></tr>
                `);
            }
        } catch (err) {
            clearTimeout(spinnerTimeout);
            $(`${tableSelector} tbody`).html(`
                <tr><td colspan="2" style="text-align:center; color:red;">Failed to load login log.</td></tr>
            `);
    ...
    fetchloginLog();
    setInterval(fetchloginLog, 10000);
